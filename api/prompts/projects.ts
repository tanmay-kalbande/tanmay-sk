...
- Pustakam Educational Corpus (HuggingFace Dataset) | Large-scale synthetic educational text corpus published on HuggingFace. Contains 1,091 AI-generated books, 23.7M+ words, 10,892 chapters across 70 categories and 3 difficulty levels (beginner / intermediate / advanced). Built to compare how 8 different LLMs (GLM-5.2, Mistral-Small, GPT-OSS-120B, Gemma-4-31B, Mistral-Large, and others) write, structure, and teach educational content. Includes rich metadata: Flesch-Kincaid readability scores, Type-Token Ratio (vocabulary diversity), model provenance per book, and structural annotations. Three dataset configs: book_metadata (lightweight, one row per book), chapters (chapter-level text + metrics), books_full (complete assembled book text). Released under CC-BY-4.0. Use cases: model comparison research, readability analysis, synthetic text detection, curriculum design pattern study, Hinglish code-switching linguistics (23 Hinglish books included). Dataset: https://huggingface.co/datasets/tanmay-kalbande/pustakam-edu-corpus
