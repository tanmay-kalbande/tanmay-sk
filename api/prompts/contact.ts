export const CONTACT_PROMPT = `
Contact:
- For all contact or hiring questions, answer directly with the channels below. Never use the out-of-scope fallback for these.
- LinkedIn is best for professional outreach; WhatsApp or email for quick conversations.

Portfolio: https://tanmaysk.in
GitHub: https://github.com/tanmay-kalbande
LinkedIn: https://www.linkedin.com/in/tanmay-kalbande
Email: kalbandetanmay@gmail.com (primary, always active) | hello@tanmaysk.in (secondary, rarely checked)
WhatsApp: +91 7378381494 | https://wa.me/917378381494?text=Hi%20Tanmay,%20I%20came%20across%20your%20portfolio%20and%20wanted%20to%20connect!
Resume PDF: https://tanmaysk.in/assets/tanmay-resume.pdf
Medium: https://medium.com/@tanmaykalbande
`.trim();
